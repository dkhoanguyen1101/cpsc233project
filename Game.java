...
public class Game {
	...
	public static void main(String[] args) {
		...
		Chara enemy = new Chara("Enemy", 2, 5, 50); //generating the generic enemy character
		...
		while(isGame) {
			if(isPlayerAlive) { //player phase
				...
				while(!didSomething) {
					...
					switch(choice) {
					case 1: didSomething = Attack(player, enemy, demoMap); break; //supplementary methods will determine if legal action was taken
			...
			if(isEnemyAlive) { //enemy phase
				...
				int[] selfPos = demoMap.getPos(enemy.getID());
			  int[] playerPos = demoMap.getPos(player.getID());
	...
	public static boolean Attack(Chara attacker, Chara receiver, map map) {
		...
		//System.out.println("" + attackerX + attackerY + receiverX + receiverY);
		if (((attackerX == receiverX + 1) || (attackerX == receiverX - 1)) || ((attackerY == receiverY + 1) || (attackerY == receiverY - 1))) {
			...
			System.out.println(attacker.getName() + " attacked " + receiver.getName() + " for " + attacker.getAttack() + " health!");
			//change boolean to true
			return true;
		} else {
			//if not within range, return false
			System.out.println(attacker.getName() + " cannot reach anybody!");
			return false;
		}
	...
	public static boolean Item(Chara user) {
	...
	public static void Move(Chara chara, map map) {
		//ask for user input (x and y positions)
		Scanner pick = new Scanner(System.in);
		System.out.println("Enter 1 to move or anything else to stay put");
		int stay = pick.nextInt();
		if(stay == 1) {
			System.out.println("Pick a y position to move to");
			int newX = pick.nextInt();
			System.out.println("Now pick an x position to move to");
			int newY = pick.nextInt();
			while (map.isLegalMove(chara.getID(), newX, newY, 4) == false) {
				System.out.println("that is not a legal move");
				System.out.println("Pick another y position to move to");
				newX = pick.nextInt();
				System.out.println("Now pick another x position to move to");
				newY = pick.nextInt();
			}
	...
	public static void charToString(Chara character) {
		System.out.println(character.getName() + "(ID: " + character.getID() +"): " + character.getHealth() + " health");
	}
}
